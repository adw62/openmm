...
SYCLBondedUtilities::SYCLBondedUtilities(OpenCLContext& context) : context(context), numForceBuffers(0), maxBonds(0), allGroups(0), hasInitializedKernels(false) {
}

void SYCLBondedUtilities::addInteraction(const vector<vector<int> >& atoms, const string& source, int group) {
    if (atoms.size() > 0) {
...
string SYCLBondedUtilities::addArgument(cl::Memory& data, const string& type) {
    arguments.push_back(&data);
    argTypes.push_back(type);
    return "customArg"+context.intToString(arguments.size());
}

string SYCLBondedUtilities::addArgument(ArrayInterface& data, const string& type) {
    return addArgument(context.unwrap(data).getDeviceBuffer(), type);
}

string SYCLBondedUtilities::addEnergyParameterDerivative(const string& param) {
...
void SYCLBondedUtilities::addPrefixCode(const string& source) {
...
void SYCLBondedUtilities::initialize(const System& system) {
...
string SYCLBondedUtilities::createForceSource(int forceIndex, int numBonds, int numAtoms, int group, const string& computeForce) {
...
void SYCLBondedUtilities::computeInteractions(int groups) {
